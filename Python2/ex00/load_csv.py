...
def load(file_path: str) -> np.array:
    ...
    with open (file_path, 'r', encoding='utf-8-sig') as csvfile:
        ...
        if not all(len(reader[0]) == len(i) for i in reader):
            ...
            exit()
        print(len(reader), len(reader[0]) )
        stripped_rows = list([[field.strip() for field in row] for row in reader[1:]])
...
def main():
    ...
    load("hello.txt")
    """ X = []
    with open(file="../csv_files/life_expectancy_years.csv", mode='r', encoding='utf-8') as csvfile:
